import urllib2
def get_page(url):
    """opens and reads urls"""
    return urllib2.urlopen(url).read()

def get_next_target(page):
    """searches container company website for each port's url"""
    start_link=page.find('<a href="/ports')
    if start_link == -1:
        return None, 0
    start_quote=page.find('"',start_link + 1)
    end_quote=page.find('"',start_quote + 1)
    url = page[start_quote + 1:end_quote]
    return url, end_quote

def print_all_links(page):
    """Pulls port urls and puts urls into a list"""
    global port_links
    port_links = []
    while True:
        url, endpos = get_next_target(page)
        if url:
            port_links_urls = "http://www.worldportsource.com" + url
            port_links.append(port_links_urls)
            page = page[endpos:]
        else:
            break
    return port_links

def lat_and_long(page):
    """searches for and returns name of port, latitude of port, and longitude of port"""
    for i in page[1:]:
        links = get_page(i)
        """find lat"""
        find_lat = links.find("Latitude:") + 86
        if find_lat == -1:
            return None, 0
        start_lat = links.find(">",find_lat + 1)
        end_lat = links.find('</td>',start_lat + 1)
        output_lat = links[start_lat + 1:end_lat].replace('&#176;','')
        """find long"""
        find_long = links.find("Longitude:") + 86
        if find_long == -1:
            return None, 0
        start_long = links.find(">",find_long + 1)
        end_long = links.find('</td>',start_long + 1)
        output_long = links[start_long + 1:end_long].replace('&#176;','')
        """find name"""
        find_name=links.find("<title>WPS")
        if find_name == -1:
            return None, 0
        start_name=links.find('-',find_name + 1)
        end_name=links.find('contact',start_name + 1)
        output_name = links[start_name + 1:end_name]
        print output_name + ", " + output_lat + ", " + output_long

links = print_all_links(get_page(raw_input("Enter a url: ")))
lat_and_long(links)


#DEMO url: http://www.worldportsource.com/shipping/poc/AAL_Shipping_Agencies_1.php







##def port_name(page):
##    find_name=page.find("<title>WPS")
##    if find_name == -1:
##        return None, 0
##    start_name=page.find('-',find_name + 1)
##    end_name=page.find('contact',start_name + 1)
##    output_name = page[start_name + 1:end_name]
##    return output_name, end_name    

##get_latitude(links)   
##get_longitude(links)


##output_lat, lastpos = get_latitude(get_page(print_all_links(links)))
##output_long, lastpos = get_longitude(get_page(print_all_links(links)))
##output_name, lastpos = port_name(get_page(print_all_links(links)))


#print output_name
#print "Latitude:" + output_lat
#print "Longitude:" + output_long
#print output_name, output_lat, output_long

##def get_latitude(page):
##    for i in page[1:]:
##        links = get_page(i)
##        find_lat = links.find("Latitude:") + 86
##        if find_lat == -1:
##            return None, 0
##        start_lat = links.find(">",find_lat + 1)
##        end_lat = links.find('</td>',start_lat + 1)
##        output_lat = links[start_lat + 1:end_lat]
##        print output_lat,
##
##def get_longitude(page):
##    for i in page[1:]:
##        links = get_page(i)
##        find_long = links.find("Longitude:") + 86
##        if find_long == -1:
##            return None, 0
##        start_long = links.find(">",find_long + 1)
##        end_long = links.find('</td>',start_long + 1)
##        output_long = links[start_long + 1:end_long]
##        print output_long
